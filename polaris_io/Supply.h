...
public:
	...
	Instance_Property_Values (int id_, shared_ptr<Instance> instatce_id_, int key_, std::string value_)
	...
	const int& getId () const {return id;}
	void setId (const int& id_) {id = id_;}
	const shared_ptr<Instance>& getInstatce_Id () const {return instatce_id;}
	void setInstatce_Id (const shared_ptr<Instance>& instatce_id_) {instatce_id = instatce_id_;}
...
private:
	...
	#pragma db id
	int id;
	shared_ptr<Instance> instatce_id;
	int key;
	std::string value;
};
